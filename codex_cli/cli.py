...
from ._task_cache import (
    save_task_cache, load_task_cache, set_current_task, get_current_task,
    resolve_task, navigate_to_task,
)
from ._init_command import cmd_init
...
def _resolve(args, config, client=None, task_arg=None):
    target = task_arg or getattr(args, "task", None)
    return resolve_task(target, config, client=client)


def _ensure_task_open(client, args, config):
    task = _resolve(args, config, client=client)
    if task:
        navigate_to_task(client, task)
    return task
...
def cmd_open_task(args, config: Config) -> int:
    task = _resolve(args, config, task_arg=args.task)
    if task is None:
        with codex_session(config) as client:
            time.sleep(3)
            task = _resolve(args, config, client=client, task_arg=args.task)
            if task is None:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="codex-cli", description="CLI for the ChatGPT Codex web UI")
